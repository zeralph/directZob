...
NJ_INLINE void nanojpeg::njFillMem(void* block, unsigned char value, int count) {
    __asm {
        mov edi, block
        mov al, value
        mov ecx, count
...
NJ_INLINE void nanojpeg::njCopyMem(void* dest, const void* src, int count) {
    __asm {
        mov edi, dest
        mov esi, src
        mov ecx, count
...
#else
extern void* nanojpeg::njAllocMem(int size);
extern void nanojpeg::njFreeMem(void* block);
extern void nanojpeg::njFillMem(void* block, unsigned char byte, int size);
extern void nanojpeg::njCopyMem(void* dest, const void* src, int size);
#endif
...
void nanojpeg::njInit(void) {
    njFillMem(&nj, 0, sizeof(nj_context_t));
}

void nanojpeg::njDone(void) {
    int i;
    for (i = 0; i < 3; ++i)
...
nj_result_t nanojpeg::njDecode(const void* jpeg, const int size) {
    njDone();
...
int nanojpeg::njGetWidth(void) { return nj.width; }
int nanojpeg::njGetHeight(void) { return nj.height; }
int nanojpeg::njIsColor(void) { return (nj.ncomp != 1); }
unsigned char* nanojpeg::njGetImage(void) { return (nj.ncomp == 1) ? nj.comp[0].pixels : nj.rgb; }
int nanojpeg::njGetImageSize(void) { return nj.width * nj.height * nj.ncomp; }
