...
void ZobEditorManager::UpdateEngine()
{
	if (m_init)
	{
		...
		std::vector<Light*> l = ZobEntity::GetEntities<Light>();
		std::vector<Light*>::const_iterator iter;
		for (iter = l.begin(); iter != l.end(); iter++)
		{
			ZobComponentSprite* zs = (*iter)->GetComponent<ZobComponentSprite>();
			if (zs)
			{
...
void ZobEditorManager::OnNewScene()
{
	...
	std::string matLightName = "editor_light_material";
	std::string texLightName = std::string("editor_light_sprite");
	m_lightImage = new Texture();
	m_lightImage->LoadFromData(texLightName, 64, 64, gLight2);
